...
class queue {
	...
	function countQueue($mailing_id) {
		if ( ACA_CMSTYPE ) {
			$database =& JFactory::getDBO();
		} else {
			global $database ;
		...
		$query = 'SELECT count(*) AS queue_nb FROM `#__acajoom_queue` WHERE `mailing_id` > 0 AND `mailing_id` = '.$mailing_id;
		$database->setQuery($query);
		$queue_nb = $database->loadRow();

		$erro = new xerr( __FILE__ , __FUNCTION__ , __CLASS__ );
		$erro->err = $database->getErrorMsg();

		return $erro->Ertrn( __LINE__ , '8502', $database, $queue_nb );
	}
	 ...
	 function insert($queue) {
		...
		$erro = new xerr( __FILE__ , __FUNCTION__ , __CLASS__ );
		$query = 'INSERT IGNORE INTO `#__acajoom_queue` (`type` , `subscriber_id` , `list_id` , `mailing_id`, `issue_nb`,' .
	...
	function insert_subs_to_mailing($list_id, $mailing_id, $send_date, $acc_level){
		...
		$query = 'INSERT IGNORE INTO `#__acajoom_queue` (`type`, `subscriber_id`, `list_id`, `mailing_id`, `issue_nb`, `send_date`, `acc_level`, `published`) ';
		$query .= 'SELECT `q`.`type`, `q`.`subscriber_id`, `q`.`list_id`, '.intval($mailing_id).', 0, \''.$send_date.'\', '.$acc_level.', 2 ';
		$query .= 'FROM `#__acajoom_queue` `q`, `#__acajoom_subscribers` `s` ';
		$query .= 'WHERE `q`.`list_id` = ' . intval($list_id) . ' ';
		$query .= 'AND `q`.`subscriber_id` = `s`.`id` AND `s`.`blacklist` = 0 AND `s`.`confirmed` = 1 ';

		$database->setQuery($query);
		$database->query();
		
		$database->setQuery('SELECT COUNT(*) FROM `#__acajoom_queue` WHERE `mailing_id` = '.intval($mailing_id));
		$number =$database->loadResult();

		$gmtTime = strtotime($send_date);
		$gmtDate = date('Y-m-d H:i:s',$gmtTime - ACA_TIME_OFFSET *60*60);
		echo acajoom::printM('blue',$number.' e-mails are in the queue for the mailing ID'.intval($mailing_id).' and will be sent at '.$send_date.' ('.$gmtDate.' GMT )');
