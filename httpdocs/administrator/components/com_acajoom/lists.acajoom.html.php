 ...
 class listsHTML {
	 ...
	 function showListingLists($lists, $action, $task, $forms, $show) {
		...
		<tr class="row<?php echo ($i % 2); ?>">
			...
			?>
			<td height="20"><center><img src="<?php echo ACA_JPATH_LIVE; ?>/administrator/images/<?php echo $img; ?>" width="12" height="12" border="0" alt="" /></center></td>
			<?php	} ?>
	...
	<?php
		if ($show['buttons']) {
			...
			if($GLOBALS[ACA.'allow_unregistered'] OR $loggedin){
				...
				$link = '.php?option=com_acajoom&act=subone&listid=' .$list->id . $item ;
